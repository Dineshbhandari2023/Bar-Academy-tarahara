...
import { Sparkles, ArrowRight, Award, ShieldCheck, MapPin, Phone, Users, Flame } from 'lucide-react';
...
export default function Hero({ theme, lang, onOpenEnroll }: HeroProps) {
  return (
    <section className={`relative overflow-hidden pt-8 pb-16 lg:pt-16 lg:pb-24 transition-colors duration-300 ${
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-7">
            <h1 className="text-4xl sm:text-6xl lg:text-7xl font-serif font-light leading-[1.1] mb-6 tracking-tight">
              {lang === 'np' ? (
                ...
                <>
                  ...
                  <span className="italic font-normal text-[#D4AF37]">Mixology</span> & Professional Barista
                </>
              )}
            </h1>
            ...
            <div className="flex flex-wrap items-center gap-4 mb-12">
              ...
              >
                <span>{lang === 'np' ? 'भर्ना आवेदन फारम' : 'Apply for Next Batch'}</span>
              ...
              >
                ...
                <span>{lang === 'np' ? 'AI रेसिपी स्टुडियो प्रयोग गर्नुहोस्' : 'Try AI Recipe Studio'}</span>
              </a>
            </div>
            ...
            <div className="pt-8 border-t border-white/10 grid grid-cols-2 sm:grid-cols-4 gap-6">
              {ACADEMY_INFO.stats.map((st, i) => (
                <div key={i} className="border-l-2 border-[#D4AF37] pl-4">
                  <p className="text-2xl sm:text-3xl font-serif font-bold text-[#D4AF37]">{st.number}</p>
                  <p className="text-[10px] uppercase tracking-wider text-white/60 mt-1">{st.label[lang]}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="lg:col-span-5 relative">
            ...
            <div className="relative rounded-2xl overflow-hidden border border-[#D4AF37]/30 shadow-2xl bg-[#111] group">
              <div className="relative h-[420px] sm:h-[480px] w-full">
                <Image
                  src="/images/hero_bar_academy_1784797795228.jpg"
              ...
              <div className="absolute top-4 right-4 bg-black/80 backdrop-blur-md border border-[#D4AF37]/50 rounded-xl p-3 flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs font-bold text-white">{lang === 'np' ? 'शीतकालीन भर्ना खुला' : 'Admissions Open'}</p>
                  <p className="text-[10px] text-white/60">{lang === 'np' ? 'सीमित सिटहरू उपलब्ध' : 'Limited Batch Seats'}</p>
                </div>
              </div>
              ...
              <div className="absolute bottom-0 inset-x-0 p-6 bg-black/80 backdrop-blur-md border-t border-white/10">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[10px] uppercase tracking-[0.2em] text-[#D4AF37] font-semibold">
                    {lang === 'np' ? 'प्रमाणित तालिम केन्द्र' : 'Certified Training Lab'}
                  </span>
                ...
                <h3 className="text-lg font-serif font-bold text-white mb-1">
                  {lang === 'np' ? 'नेपालको उत्कृष्ट बार एकेडेमी' : 'Bar Academy Tarahara Practical Labs'}
                </h3>
            ...
            <div className="hidden sm:flex absolute -bottom-6 -left-6 bg-[#0a0a0a] border border-[#D4AF37] p-4 rounded-xl shadow-xl items-center gap-3">
              ...
              <div>
                <p className="text-xs font-bold text-white">{lang === 'np' ? '१००% रोजगारी सहजीकरण' : '100% Job Placement Assistance'}</p>
                <p className="text-[10px] text-white/50">{lang === 'np' ? 'नेपाल, युएई, कतार र युरोप' : 'Nepal, Dubai, Qatar, Maldives & Europe'}</p>
              </div>
            </div>
